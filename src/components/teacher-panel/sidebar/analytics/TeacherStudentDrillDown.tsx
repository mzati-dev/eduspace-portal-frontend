// components/teacher/analytics/TeacherStudentDrillDown.tsx
...
import { TeacherStudentDetail } from './TeacherAnalyticsTypes';

interface TeacherStudentDrillDownProps {
...
const TeacherStudentDrillDown: React.FC<TeacherStudentDrillDownProps> = ({
    student,
    loading,
    onBack,
    onExportPDF,
    ...
    const getStatusColor = (status: string) => {
        if (status === 'Failing' || status === 'Critical') return 'bg-red-100 text-red-700';
        if (status === 'Needs Support' || status === 'High') return 'bg-orange-100 text-orange-700';
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-wrap justify-between items-start gap-4">
                    <div>
                        ...
                        <p className="text-slate-500">
                            📚 {student.className} | Exam No: {student.examNumber}
                        </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                    <p className="text-sm text-slate-500">Current Marks</p>
                    <p className={`text-2xl font-bold ${getScoreColor(student.currentMarks)}`}>
                        {student.currentMarks}%
                    </p>
                    <p className="text-xs text-red-500">▼ Below class average</p>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                    <p className="text-sm text-slate-500">Current Attendance</p>
                    <p className={`text-2xl font-bold ${getScoreColor(student.currentAttendance)}`}>
                        {student.currentAttendance}%
                    </p>
                    <p className="text-xs text-red-500">▼ Needs improvement</p>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h3 className="font-semibold text-slate-800 mb-4">Subject-wise Breakdown (Current Term)</h3>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {student.subjectBreakdown.map((subject, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td className={`px-3 py-2 font-medium ${getScoreColor(subject.marks)}`}>
                                        {subject.marks}%
                                    </td>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {student.historical.map((record, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td className={`px-3 py-2 font-medium ${getScoreColor(record.marks)}`}>
                                        {record.marks}%
                                    </td>
...
export default TeacherStudentDrillDown;
...
//                         </div>
//                         <p className="text-slate-500">
//                             📚 {student.className} | Batch: {student.batch} | Roll No: {student.rollNo}
//                         </p>
//                         <p className="text-slate-500 text-sm">👩‍🏫 Class Teacher: {student.classTeacher}</p>
//                     </div>
//                     <div className="flex gap-2">

...
//     return (
//         <>
//             {/* Back to your exact original wrapper - NO sticking */}
//             <div className="flex flex-wrap border-b border-slate-200 bg-white">
//                 {tabs.map((tab) => (
//                     <button
//                         key={tab.id}
//                         onClick={() => setActiveTab(tab.id)}
//                         className={`flex-1 min-w-[120px] sm:min-w-[150px] px-2 sm:px-4 py-4 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 outline-none
//                             ${activeTab === tab.id
//                                 ? 'text-blue-700 border-b-4 border-blue-600 bg-blue-50/50'
//                                 : 'text-slate-500 border-b-4 border-transparent hover:text-blue-600 hover:bg-slate-50 hover:border-slate-300'
//                             }`}
//                     >
//                         <tab.icon className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${activeTab === tab.id ? 'text-blue-600' : 'text-slate-400'}`} />

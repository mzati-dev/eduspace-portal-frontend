...
import { API_BASE_URL, fetchAttendanceAnalytics, fetchClassComparisons } from '@/services/attendanceService';
...
const AttendanceAnalytics: React.FC<AnalyticsProps> = ({ classId, className, students, showMessage, allClasses = [] }) => {
    ...
    const fetchClassComparisonsList = async () => {
        try {
            const data = await fetchClassComparisons();
            setClassComparisons(data);
        } catch (error) {
            console.error('Failed to fetch class comparisons:', error);
            setClassComparisons([]);
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Class Comparison Section - ALWAYS SHOW */}

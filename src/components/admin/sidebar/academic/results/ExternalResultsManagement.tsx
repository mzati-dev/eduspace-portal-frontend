...
const ExternalResultsManagement: React.FC<ExternalResultsManagementProps> = ({
    ...
    // State for Secondary Selections
    const [secondarySelections, setSecondarySelections] = useState<Record<string, { school: string; status: string }>>({});

    // State for University Selections
    const [universitySelections, setUniversitySelections] = useState<Record<string, { university: string; program: string; status: string }>>({});
    ...
    // Load secondary selections from localStorage
    useEffect(() => {
        const schoolId = getCurrentSchoolId();
        const saved = localStorage.getItem(`secondarySelections_${schoolId}`);
        if (saved) {
            try {
                setSecondarySelections(JSON.parse(saved));
            } catch (e) {
                console.error('Failed to load secondary selections', e);
            }
        }
    }, []);

    // Load university selections from localStorage
    useEffect(() => {
        const schoolId = getCurrentSchoolId();
        const saved = localStorage.getItem(`universitySelections_${schoolId}`);
        if (saved) {
            try {
                setUniversitySelections(JSON.parse(saved));
            } catch (e) {
                console.error('Failed to load university selections', e);
            }
        }
    }, []);
    ...
    // Save secondary selections
const saveSecondarySelections = () => {
    const schoolId = getCurrentSchoolId();
    localStorage.setItem(`secondarySelections_${schoolId}`, JSON.stringify(secondarySelections));
    showMessage('Secondary selections saved successfully!');
};

// Save university selections
const saveUniversitySelections = () => {
    const schoolId = getCurrentSchoolId();
    localStorage.setItem(`universitySelections_${schoolId}`, JSON.stringify(universitySelections));
    showMessage('University selections saved successfully!');
};

// Update secondary selection for a student
const updateSecondarySelection = (studentId: string, field: 'school' | 'status', value: string) => {
...
// Update university selection for a student
const updateUniversitySelection = (studentId: string, field: 'university' | 'program' | 'status', value: string) => {
    setUniversitySelections(prev => ({
        ...prev,
        [studentId]: {
            ...prev[studentId],
            [field]: value,
    ...
    return (
        <div className="space-y-6">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-600">Enter secondary school selections for Standard 8 students</p>
            </div>
                ...
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredStudents.map((student) => {
                            const studentSelection = secondarySelections[student.id] || { school: '', status: 'Pending' };
                            return (
                                <tr key={student.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3">
                                        <input
                                            type="text"
                                            value={studentSelection.school}
                                            onChange={(e) => updateSecondarySelection(student.id, 'school', e.target.value)}
                                            placeholder="Enter school name"
                                            className="w-full px-3 py-1 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm"
                                        />
                                    </td>
                                    <td className="px-4 py-3 text-center">
                                        <select
                                            value={studentSelection.status}
                                            onChange={(e) => updateSecondarySelection(student.id, 'status', e.target.value)}
                                            className="px-3 py-1 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
                                        >
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        {studentSelection.school && studentSelection.status === 'Selected' && (
            ...
            <div className="flex justify-end">
                <button
                    onClick={saveSecondarySelections}
    ...
    return (
        <div className="space-y-6">
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <p className="text-sm text-purple-600">Enter university selections for Form 4 students</p>
            </div>
                ...
                <table className="w-full">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600">Program/Course</th>
                            <th className="text-center px-4 py-3 text-sm font-semibold text-slate-600">Status</th>
                            <th className="text-center px-4 py-3 text-sm font-semibold text-slate-600">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {filteredStudents.map((student) => {
                            const studentSelection = universitySelections[student.id] || { university: '', program: '', status: 'Pending' };
                            return (
                                <tr key={student.id} className="hover:bg-slate-50">
                                    <td className="px-4 py-3 font-medium text-slate-800">{student.name}</td>
                                    <td className="px-4 py-3 font-mono text-sm text-indigo-600">{student.examNumber}</td>
                                    <td className="px-4 py-3">
                                        <input
                                            type="text"
                                            value={studentSelection.university}
                                            onChange={(e) => updateUniversitySelection(student.id, 'university', e.target.value)}
                                    ...
                                    <td className="px-4 py-3">
                                        <input
                                            ...
                                            onChange={(e) => updateUniversitySelection(student.id, 'program', e.target.value)}
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <select
                                            value={studentSelection.status}
                                            onChange={(e) => updateUniversitySelection(student.id, 'status', e.target.value)}
                                            className="px-3 py-1 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
                                        >
                                            <option value="Pending">Pending</option>
                                            <option value="Selected">Selected</option>
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        {studentSelection.university && studentSelection.status === 'Selected' && (
                                            <span className="px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">
                                                Saved
                                            </span>
                                        )}
            ...
            <div className="flex justify-end">
                <button
                    onClick={saveUniversitySelections}
                    className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium flex items-center gap-2"
                >
                    Save All Selections
                </button>

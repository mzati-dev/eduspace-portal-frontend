        ...
        try {
            ...
            const studentsWithCalculations = classResults.map(student => {
                ...
                return {
                    ...
                    calculatedGrade: calculateGrade(average, activeConfig?.pass_mark, false, className),
                    calculatedStatus: calculateGrade(average, activeConfig?.pass_mark, false, className) === 'F' ? 'Failed' : 'Passed'
                };
            });
            ...
            const tableBody = sortedStudents.map((student, index) => {
                ...
                const subjectCols = filteredSubjects.map((subj) => {
                    ...
                    if (activeAssessmentType === 'overall') {
                        ...
                        const grade = calculateGrade(finalScore, activeConfig?.pass_mark, false, className);
                        ...
                        if (isAbsent) {
                            ...
                            const grade = calculateGrade(score, activeConfig?.pass_mark, false, className);
                            return `${score} (${grade})`;
                        }
                        return '-';
                    }

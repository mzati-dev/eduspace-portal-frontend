...
const CompareTerms: React.FC<CompareTermsProps> = ({
    ...
    const selectedClassName = classes.find(c => c.id === selectedClassFilter)?.name;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="text-center flex-1">
                    ...
                    <p className="text-slate-500">
                        Compare performance across two terms
                        {selectedClassFilter !== 'all' && selectedClassName && (
                            <span className="ml-2 text-indigo-600">- {selectedClassName}</span>
                        )}
                    </p>
                </div>
                <button onClick={onExportReport} className="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                {/* Class Filter - Only show if classes exist */}
                {classes.length > 0 && (

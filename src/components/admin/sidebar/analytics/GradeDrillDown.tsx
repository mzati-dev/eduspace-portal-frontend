...
// DUMMY STUDENTS DATA - Shows 10 sample students per grade
...
const GradeDrillDown: React.FC<GradeDrillDownProps> = ({ gradeName, students, onViewStudent, onBack }) => {
    ...
    useEffect(() => {
        setTimeout(() => {
            ...
            if (students && students.length > 0) {
                filtered = students
                    .filter(s => s.class?.name === gradeName)
                    .map((s, idx) => ({
            ...
            if (filtered.length === 0 && DUMMY_STUDENTS[gradeName]) {
                filtered = DUMMY_STUDENTS[gradeName];
            }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <p className="text-slate-500 mt-1">{gradeStudents.length} students in this grade</p>
            </div>
...
//                     examNumber: s.examNumber || `STU${String(idx + 1).padStart(4, '0')}`,
//                     grade: gradeName,
//                     attendance: 50 + Math.random() * 45,
//                     catScore: 45 + Math.random() * 45,
//                     fails: Math.floor(Math.random() * 3),
//                     currentMarks: 40 + Math.random() * 50,
//                     riskLevel: Math.random() > 0.8 ? 'critical' : Math.random() > 0.5 ? 'high' : 'medium'
//                 }));
//             setGradeStudents(filtered);
//             setLoading(false);
...
//                     <table className="w-full">
//                         <thead className="bg-slate-50">
//                             <tr><th className="text-left px-4 py-3">Student</th><th>Exam No</th><th>Att</th><th>CAT</th><th>Marks</th><th>Fails</th><th>Risk</th><th>Action</th></tr>
//                         </thead>
//                         <tbody>
//                             {gradeStudents.map(s => (
//                                 <tr key={s.id} className="hover:bg-slate-50">
//                                     <td className="px-4 py-3 font-medium">{s.name}</td>
//                                     <td className="px-4 py-3 text-indigo-600">{s.examNumber}</td>
//                                     <td className="px-4 py-3">{Math.round(s.attendance)}%</td>
//                                     <td className="px-4 py-3">{Math.round(s.catScore)}%</td>
//                                     <td className="px-4 py-3">{Math.round(s.currentMarks)}%</td>
//                                     <td className="px-4 py-3">{s.fails}</td>
//                                     <td className="px-4 py-3">
//                                         <span className={`px-2 py-1 rounded-full text-xs ${s.riskLevel === 'critical' ? 'bg-red-100 text-red-700' : s.riskLevel === 'high' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}`}>
//                                             {s.riskLevel.toUpperCase()}
//                                         </span>
//                                     </td>
//                                     <td className="px-4 py-3">
//                                         <button onClick={() => onViewStudent(s.id)} className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-sm">View</button>
//                                     </td>
//                                 </tr>
//                             ))}
//                         </tbody>
